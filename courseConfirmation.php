...
//---------GRABBING LAST EVAL ID TO INITIALIZE A ROW CONNECTED TO AN EVENT
    $query_RecordsetEvalID = "SELECT evaluations.evaluationID FROM metro.evaluations ORDER BY evaluations.evaluationID DESC LIMIT 1;";
    $RecordsetEvalID = mysql_query($query_RecordsetEvalID, $metro) or die(mysql_error());
    $row_RecordsetEvalID = mysql_fetch_assoc($RecordsetEvalID);
    $row_RecordsetEvalID['evaluationID'] += 1; //incrementing last id
//-------------------------------------------------
...
//------------EVENT INFO-------------------------- INCLUDE eventDateTime (what is this field), donelocation, doneroomNum, donenumParticipants, **status, **evaluationId***
    ...
    $query_RecordsetEvent = "INSERT INTO metro.events (eventID, teacherID, location, roomNum, numParticipants, eventDateTime, dateOfRequest, status, evaluationID, programID)
                            ...
                            '". $date_requested . " " .$time_requested."', '". date("Y-m-d H-i-s") ."', 'Pending Artist', '".$row_RecordsetEvalID['evaluationID']."', '".$row_RecordsetProgramID['programID']."');";
    //echo $query_RecordsetEvent;
    $RecordsetEvent = mysql_query($query_RecordsetEvent, $metro) or die(mysql_error());
//---------------------------------------------------------


//---------EVAL INFO-------Inserting eventID into evalulations table with null data so event info
//-------------------------is connected to evaluations
    //inserting event info into events table
    $query_RecordsetEvalInfo = "INSERT INTO metro.evaluations (evaluationID, eventID)
                            VALUES ('".$row_RecordsetEvalID['evaluationID']."', '".$row_RecordsetEventID['eventID']."');";
    //echo $query_RecordsetEvalInfo;
    $RecordsetEvaluations = mysql_query($query_RecordsetEvalInfo, $metro) or die(mysql_error());
//-------------------------------------------------
...
$bodyTeacher             = "<html><body><p>this is a paragraph for the teacher</p></body></htmL>"; //contents to send
...
$addressTeacher = $email; //send to address
$mailTeacher->AddAddress($addressTeacher, $fullname); //adding name to email address
...
if(!$mailTeacher->Send()) {
  echo "The email did not send correctly. Please push the \"back\" button and wait a few moments to resubmit your information. Thank you!";
  exit();
} else {
